/* Query SQL
Using this dataset, show the rolling 3-day average of total automation runs per day for March 15 */

SELECT
  d1.run_date,
  AVG(d2.daily_runs) AS rolling_3day_avg_runs
FROM (
    SELECT DATE(transaction_time) AS run_date, COUNT(*) AS daily_runs
    FROM transactions
...
JOIN (
  ...
  ON d2.run_date BETWEEN DATE_SUB(d1.run_date, INTERVAL 2 DAY)
                     AND d1.run_date
WHERE d1.run_date = '2021-03-15'
GROUP BY d1.run_date;



/* Query SQL
Using this dataset, show the rolling 3-day average of total automation runs per day for Jan 15 */
...
/*
Schema SQl: 
Synthetic telemetry for an internal automation engine.
Each row represents a single automation run triggered by the system.
*/
